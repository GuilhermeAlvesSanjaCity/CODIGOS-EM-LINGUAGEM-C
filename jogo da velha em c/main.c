#include <stdio.h>    // biblioteca da linguagem c padrao
#include <stdlib.h>   // biblioteca da lingaugem c padrao de entrada e saida de dados
#include <time.h>     // bibilioteca q serve para manipular datas
#include <conio.h>    // usada para pausar o terminal



void inicio_do_jogo(char Mat[3][3]) {                // define as duas matrizes pois sera utilizado o conceito do x,y numa matriz de 3x3
 int L,C;
    do {
     ...
     // while(L<1 || L>3 || C<1 || C>3 || Mat[L-1][C-1]!=' ');  // condicional onde l e c tem q receber valores entre 0 e 2
      while(L<0 || L>2 && C<0 || C>2 || Mat[L][C]!=' '); // entre 0 e 2

     // Mat[L-1][C-1]='x';
       Mat[L][C]='x';  // recebe a caracter x onde for inidicado os valores
}

void mostrar_matriz(char x[3][3]){  // funcao para exibir o inicio da matriz na tela onde nao possuem as cordenadas ainda
